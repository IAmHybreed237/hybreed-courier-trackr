...
if (isset($_GET['tnum']) && $_GET['tnum'] != "") {
    ...
    if (mysqli_num_rows($select) > 0) {
        ...
        if ($hasCustomDate) {
            $history_stmt = mysqli_prepare($link, "SELECT status, location, remarks, custom_date, created_at FROM shipment_history WHERE tracking_number = ? ORDER BY COALESCE(custom_date, created_at) DESC");
        } else {
            $history_stmt = mysqli_prepare($link, "SELECT status, location, remarks, created_at FROM shipment_history WHERE tracking_number = ? ORDER BY created_at DESC");
        }
        ...
        $logoCandidates = array(
            ...
            $baseDir.'/logo/logo-placeholder.svg'
        );
...
<head>
    ...
    <title>Shipment Receipt - Hybreed Courier</title>
    </head>
    <body>
        <div class="receipt">
            <div class="header">
