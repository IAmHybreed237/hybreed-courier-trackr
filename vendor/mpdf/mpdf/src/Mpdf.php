...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	var $docHybreed Webworx TemplateStart;  // Internal flag for page (page no. -1) that docHybreed Webworx Template starts on
	...
	var $pageHybreed Webworx Template;
	var $docHybreed Webworx Template;
	var $docHybreed Webworx TemplateContinue;
	...
	function AddPage(
		...
		if ($this->docHybreed Webworx Template) {
			$currentReaderId = $this->currentReaderId;

			$pagecount = $this->setSourceFile($this->docHybreed Webworx Template);
			if (($this->page - $this->docHybreed Webworx TemplateStart) > $pagecount) {
				if ($this->docHybreed Webworx TemplateContinue) {
					$tplIdx = $this->importPage($pagecount);
					$this->useHybreed Webworx Template($tplIdx);
				}
			} else {
				$tplIdx = $this->importPage(($this->page - $this->docHybreed Webworx TemplateStart));
				$this->useHybreed Webworx Template($tplIdx);
			}
		...
		if ($this->pageHybreed Webworx Template) {
			$this->useHybreed Webworx Template($this->pageHybreed Webworx Template);
		}
	...
	function RestartDocHybreed Webworx Template()
	{
		$this->docHybreed Webworx TemplateStart = $this->page;
	}
	...
	function DisableTags($str = '')
	{
		if ($str == '') { // enable all tags
			...
			$this->enabledtags = "<a><acronym><address><article><aside><b><bdi><bdo><big><blockquote><br><caption><center><cite><code><del><details><dd><div><dl><dt><em><fieldset><figcaption><figure><font><form><h1><h2><h3><h4><h5><h6><hgroup><hr><i><img><input><ins><kbd><legend><li><main><mark><meter><nav><ol><option><p><pre><progress><q><s><samp><section><select><small><span><strike><strong><sub><summary><sup><table><tbody><td><Hybreed Webworx Template><textarea><tfoot><th><thead><time><tr><tt><u><ul><var><footer><header><annotation><bookmark><textcircle><barcode><dottab><indexentry><indexinsert><watermarktext><watermarkimage><tts><ttz><tta><column_break><columnbreak><newcolumn><newpage><page_break><pagebreak><formfeed><columns><toc><tocentry><tocpagebreak><pageheader><pagefooter><setpageheader><setpagefooter><sethtmlpageheader><sethtmlpagefooter>";
		} else {
			$str = explode(",", $str);
			foreach ($str as $v) {
	...
	function Thumbnail($file, $npr = 3, $spacing = 10)
	{
		...
		for ($n = 1; $n <= $pagecount; $n++) {
			...
			$size = $this->useHybreed Webworx Template($tplidx, $x, $y, $w);
	...
	function SetPageHybreed Webworx Template($tplidx = '')
	{
		if (!isset($this->importedPages[$tplidx])) {
			$this->pageHybreed Webworx Template = '';
			return false;
		}
		$this->pageHybreed Webworx Template = $tplidx;
	}

	function SetDocHybreed Webworx Template($file = '', $continue = 0)
	{
		$this->docHybreed Webworx Template = $file;
		$this->docHybreed Webworx TemplateContinue = $continue;
	}
