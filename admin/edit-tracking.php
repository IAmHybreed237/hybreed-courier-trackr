        ...
        if (preg_match('/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/', $update_datetime)) {
	     ...
	     if (empty($sendersname) || empty($senderscontact) || empty($sendersmail) || empty($sendersaddress) || empty($receiversname) || empty($receiverscontact) || empty($receiversmail) || empty($receiversaddress) || empty($status) || empty($dispatchl) || empty($dispatch) || empty($delivery) ) {
	       echo "<script>alert('Check !!! Some fields are empty')</script>";
	     	 ...
	     	 $old_status_query = mysqli_query($link, "SELECT status FROM tracking WHERE tracking_number = '$tnumbs'");
	     	 $old_status_row = mysqli_fetch_assoc($old_status_query);
	     	 ...
	     	 if ($update) {
	     	     ...
	     	     if ($should_log) {
	     	         ...
	     	         if ($colCheck && mysqli_num_rows($colCheck) > 0) {
	     	           $history_insert = mysqli_query($link, "INSERT INTO shipment_history (tracking_number, status, location, remarks, custom_date) VALUES ('$tnumbs', '$status', '$location_for_history', '$remarks', '$customDateTime')");
	     	         } else {
	     	           $history_insert = mysqli_query($link, "INSERT INTO shipment_history (tracking_number, status, location, remarks, created_at) VALUES ('$tnumbs', '$status', '$location_for_history', '$remarks', '$customDateTime')");
	     	         }
