...
$sendersname = $senderscontact = $sendersmail = $sendersaddress = $receiversname = $receiverscontact = $receiverscontact = $receiversmail =  $receiversaddress = $status = $dispatchl = $dispatch = $delivery = $desc = $initial_datetime = "";
    ...
    if (empty($sendersname) || empty($senderscontact) || empty($sendersmail) || empty($sendersaddress) || empty($receiversname) || empty($receiverscontact) || empty($receiversmail) || empty($receiversaddress) || empty($status) || empty($dispatchl) || empty($dispatch) || empty($delivery) ) {
       ...
       $insert_stmt = mysqli_prepare($link, "INSERT INTO tracking (tracking_number, sender_name, sender_contact, sender_email, sender_address, status, dispatch_location, receiver_email, receiver_name, receiver_contact, receiver_address, dispatch_date, delivery_date, pdesc) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
       mysqli_stmt_bind_param($insert_stmt, "ssssssssssssss", $tnumbs, $sendersname, $senderscontact, $sendersmail, $sendersaddress, $status, $dispatchl, $receiversmail, $receiversname, $receiverscontact, $receiversaddress, $dispatch, $delivery, $desc);
       mysqli_stmt_execute($insert_stmt);
       $insert = mysqli_stmt_affected_rows($insert_stmt);
       ...
       if ($insert > 0) {
         ...
         if ($colCheck && mysqli_num_rows($colCheck) > 0) {
           ...
           mysqli_stmt_bind_param($history_stmt, "sssss", $tnumbs, $status, $dispatchl, 'Shipment created', $customDateTime);
         } else {
